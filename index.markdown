...
| `{% raw %}{{todayWeather}}{% endraw %}` | The weather forecast for today, obtained from [open-meteo.com](https://open-meteo.com) |
...
| `{% raw %}{{lastEmail}}{% endraw %}` | The subject, sender, and content of the most recently received email in Mail.app |
| `{% raw %}{{lastNote}}{% endraw %}` | The text of the most recently edited note in Notes.app |
| `{% raw %}{{musicTracks}}{% endraw %}` | The list of track titles in Music.app |
| `{% raw %}{{safariBookmarks}}{% endraw %}` | List of bookmarks in Safari |
...
| `{% raw %}{{time}}{% endraw %}` | The current name (hours and minutes) |
| `{% raw %}{{todayEvents}}{% endraw %}` | Upcoming events over the next day |
| `{% raw %}{{weekEvents}}{% endraw %}` | Upcoming events over the next 7 days |
| `{% raw %}{{monthEvents}}{% endraw %}` | Upcoming events over the next month |
| `{% raw %}{{yearEvents}}{% endraw %}` | Upcoming events over the next year |
| `{% raw %}{{todayReminders}}{% endraw %}` | Upcoming reminders of the next day |
| `{% raw %}{{weekReminders}}{% endraw %}` | Upcoming reminders over the next 7 days |
| `{% raw %}{{monthReminders}}{% endraw %}` | Upcoming reminders over the next month |
...
| `{% raw %}{{currentApplication}}{% endraw %}` | The name of the current application |
| `{% raw %}{{currentDirectory}}{% endraw %}` | The path of the current directory |
| `{% raw %}{{currentTabText}}{% endraw %}` | The text content of the active tab of the active browser |
| `{% raw %}{{currentTrack}}{% endraw %}` | The title of the track/stream currently playing in Music.app |
| `{% raw %}{{currentURL}}{% endraw %}` | The current URL of the active tab of the active browser |
| `{% raw %}{{fileAICommands}}{% endraw %}` | The list of all custom PromptLab commands |
...
| `{% raw %}{{homedir}}{% endraw %}` | The user's home directory |
| `{% raw %}{{hostname}}{% endraw %}` | The computer's hostname |
...
| Compose Response | Compose a response to the following files in the style of an email. Sign the email with the name "{% raw %}{{user}}{% endraw %}" |
...
| Summarize Clipboard | Summarize the following text, providing specific details. ###{% raw %}{{clipboardText}}{% endraw %}### |
| Summarize Current Tab | Based on the following text obtained from the active browser tab, what am I looking at? In your answer, discuss the content and meaning of the website. If the URL or the text are blank, report an error. For context, here is the url: {% raw %}{{currentURL}}{% endraw %} And here is the text: ###{% raw %}{{currentTabText}}{% endraw %}### |
| Summarize Last Email | Summarize my most recent email and discuss its significance. I am the recipient. Format the response as a paragraph directed to me. Here is the email: ###{% raw %}{{lastEmail}}{% endraw %}### |
...
| Suggest Similar Songs | Suggest songs similar to {currentTrack}}{% endraw %}. Use "Songs Similar To {% raw %}{{currentTrack}}{% endraw %}" as the header for the output. For each suggestion, provide a brief explanation.{% raw %}{{END}}{% endraw %} |
