...
export default function Command() {
    ...
    const basePrompt = "What overlaps in content or ideas exists between the following texts. Format the response as markdown.\n"
    const fileContentsString = fileContents.join("\n")

    const { data, isLoading } = useUnstableAI(basePrompt + fileContentsString, { execute: fileContents.length > 2 });

    if (commandError) {
        showToast({ title: "Failed Overlap Analysis", message: commandError, style: Toast.Style.Failure })
        popToRoot()
        return
    }
