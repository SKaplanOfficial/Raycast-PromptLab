...
import { Insight, InsightManager, PersistentVariable } from "../utils/types";
 ...
 */
export const readInsight = async (insightId: string): Promise<Insight | undefined> => {
 ...
 */
export const allInsights = async (): Promise<Insight[]> => {
  ...
  try {
    ...
    for (const insightFile of insightFiles) {
      const insight = await readInsight(insightFile.replace(".json", ""));
      if (insight) {
 ...
 * @returns A promise resolving to an array of insights sorted by date descending.
 */
export const getInsights = async (amount?: number, tag?: string): Promise<Insight[]> => {
  if (!amount && !tag) {
    return allInsights();
  }
 ...
 */
export const relatedInsights = async (insight: Insight): Promise<Insight[]> => {
 ...
 */
export const connectedInsights = async (insight: Insight): Promise<Insight[]> => {
  const insights = await allInsights();
